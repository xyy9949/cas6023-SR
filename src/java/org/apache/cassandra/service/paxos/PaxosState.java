...
public class PaxosState
{
    ...
    public static PrepareResponse prepare(Commit toPrepare)
    {
        synchronized (lockFor(toPrepare.key))
        {
            ...
            {
                logger.info("[DIKAI] promising ballot {}", toPrepare.ballot);
            ...
            {
                logger.info("[DIKAI] promise rejected; {} is not sufficiently newer than {}", toPrepare, state.inProgressCommit);
    ...
    public static Boolean propose(Commit proposal)
    {
        synchronized (lockFor(proposal.key))
        {
            ...
            {
                logger.info("[DIKAI] accepting proposal {}", proposal);
                Tracing.trace("accepting proposal {}", proposal);
                SystemKeyspace.savePaxosProposal(proposal);
                return true;
            }

            logger.info("[DIKAI] accept requested for {} but inProgress is now {}", proposal, state.inProgressCommit);
    ...
    public static void commit(Commit proposal)
    {
        ...
        logger.info("[DIKAI] committing proposal {}", proposal);
